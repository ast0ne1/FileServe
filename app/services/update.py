...
logger = logging.getLogger("fileserve.update")
...
def check_latest(db: Session, download: bool = True) -> dict:
    ...
    if not repo:
        payload = {
            "ok": False,
            "newer": False,
            "message": "Set the GitHub repository (owner/FileServe) before checking for updates.",
        }
    ...
    if not payload["newer"]:
        payload["message"] = f"FileServe is up to date ({__version__})."
        return _save_check(db, payload)
...
def _download_zip(zip_url: str, tag: str) -> Path:
    ...
    dest = staging / f"fileserve-{tag}.zip"
...
def _replace_file(source: Path, dest: Path) -> None:
    ...
    tmp = dest.with_name(f".{dest.name}.fileserve-tmp")
    try:
        if tmp.exists():
            tmp.unlink()
...
def apply_zip(zip_path: Path, expected_tag: str = "") -> dict:
    ...
    if root is None:
        raise ValueError("Could not find the FileServe app folder in the zip.")
...
def schedule_restart() -> None:
    Thread(target=_restart_soon, daemon=True, name="fileserve-restart").start()
...
def _restart_soon() -> None:
    ...
    time.sleep(1.2)
    service = Path("/etc/systemd/system/fileserve.service")
    if service.exists():
        subprocess.Popen(["sudo", "systemctl", "restart", "fileserve"], close_fds=True)
        return
